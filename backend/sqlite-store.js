...
function normalizeTaskRow(r) {
  ...
  if (hasNewDates) {
    return {
      ...
      currentStage: Number(r.currentStage ?? 0),
      updates: safeJsonParse(updatesJson, []),
  ...
  const run = sqlDb.transaction(() => {
    ...
    for (const t of data.tasks || []) {
      ...
      insTask.run({
        ...
        title: t.title ?? "",
        batchDate: anchor,
        currentStage: Number(t.currentStage ?? 0),
        updatesJson: JSON.stringify(Array.isArray(t.updates) ? t.updates : []),
